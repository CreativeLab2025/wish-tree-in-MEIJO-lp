...
const AppleRotation: React.FC<AppleRotationProps> = ({
  size = 'medium',
  rotationSpeed = 1,
  scaleRange = [0.5, 1.5],
  className = '',
  ...
  return (
    ...
    >
      ...
      {process.env.NODE_ENV === 'development' && animation.isVisible && (
        <div className="absolute bottom-2 left-2 bg-black bg-opacity-50 text-white text-xs p-1 rounded">
          Frame: {animation.currentFrame}<br />
          Progress: {Math.round(animation.scrollProgress * 100)}%<br />
          Scale: {animation.scale.toFixed(2)}
        </div>
      )}
    </div>
  );
